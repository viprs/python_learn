...
1.编写一个程序，要求输出100个1~20以内的随机数

2.有一个tuple，内容是多个字符串如：
...
面向过程：
1. 面向过程就是分析出解决问题所需要的步骤,
2. 然后用函数把这些步骤一步一步实现,
3. 使用的时候一个一个依次调用就可以了

缺点：必须知道前人是如何处理数据，如何定义的方法，C不支持面向对象

面向对象：
1. 面向对象是把构成问题事务分解成各个对象，
2. 建立对象的目的不是为了完成一个步骤，
3. 而是为了描叙某个事物在整个解决问题的步骤中的行为。

把数据和方整合成为一个对象，更适合开发者模块化工作，C++、Python、Java
...
def test_tuple():
    """
    2.用户输入一个单词，如果在tuple里，就返回处于第几个位置（从0开始），
